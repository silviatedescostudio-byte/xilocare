...
/**
 * SHELLY API ROUTE - DATI REALI DA VARIABILI D'AMBIENTE
...
export async function GET() {
  ...
  const DEVICE_ID = process.env.SHELLY_DEVICE_ID || process.env.ID_DISPOSITIVO_SHELLY;
  const AUTH_KEY = process.env.SHELLY_AUTH_KEY;

  // Se manca configurazione, ritorna errore 500
  if (!SERVER || !DEVICE_ID || !AUTH_KEY) {
  ...
  try {
    ...
    const url = `${SERVER}/device/status?id=${DEVICE_ID}&auth_key=${AUTH_KEY}&_=${nocache}`;
    ...
    // temperatura da device_status['temperature:0']?.tC
    // umidità da device_status['humidity:0']?.rh
